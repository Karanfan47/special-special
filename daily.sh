...
# Ensure the script is run with sudo for systemd setup
if [ "$EUID" -ne 0 ]; then
    echo -e "${RED}❌ This script must be run as root (sudo) to set up the systemd service.${NC}"
    exit 1
fi
...
setup_downloaders() {
    ...
    cat << 'INNER_EOF' > $HOME/video_downloader.py
import yt_dlp
import os
import sys
import time
...
INNER_EOF
    ...
    cat << 'INNER_EOF' > $HOME/pixabay_downloader.py
import requests
import os
import sys
import time
...
INNER_EOF
    ...
    cat << 'INNER_EOF' > $HOME/pexels_downloader.py
import requests
import os
import sys
import time
